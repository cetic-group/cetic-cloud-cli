...
## Planificateur marche/arrêt (depuis v0.41.0)

`cetic schedule` programme l'arrêt et le redémarrage automatiques d'une ressource
selon des fenêtres hebdomadaires (éteindre la nuit, les week-ends…). La ressource
est **arrêtée**, jamais détruite : le stockage est conservé, elle redémarre à
l'identique en dehors des fenêtres. Facturation à part, forfaitaire par instance.

Cibles (une seule par planning, flags exclusifs) — acceptent un ID **ou** un nom :

| Flag | Ressource ciblée |
|---|---|
...
| `--db-instance UUID` | une instance de base de données managée |
...
- Plusieurs `--off` = plusieurs fenêtres (ex. une par nuit de semaine + le week-end).

Un arrêt de moins d'une heure ou plus de 2 cycles marche/arrêt par jour est refusé
par l'API (aucun gain : l'économie est facturée à l'heure) — le message d'erreur
métier est affiché tel quel.

```bash
# Éteindre une VM tout le week-end (vendredi 20h → lundi 08h)
cetic schedule create weekend --vm web-01 --off "fri:20-mon:08"

# Éteindre un VM scale set chaque nuit de semaine, fuseau explicite
cetic schedule create nuits --vm-scale-set fleet \
    --off "mon:22-tue:07" --off "tue:22-wed:07" --off "wed:22-thu:07" \
    --off "thu:22-fri:07" --timezone Europe/Paris

# Éteindre un pool de nœuds Kubernetes la nuit
cetic schedule create pool-nuit --ccks-node-pool prod gpu --off "mon:20-tue:08"

cetic schedule list                    # état ON/OFF + coût estimé mensuel par planning
cetic schedule get weekend             # détail (fenêtres, état, dernière transition)
cetic schedule update weekend --off "fri:19-mon:09"   # remplace TOUTES les fenêtres
cetic schedule disable weekend         # suspendre sans supprimer
cetic schedule enable weekend          # réactiver
...
# Planificateur marche/arrêt (depuis v0.41.0) — éteindre nuits + week-ends
cetic schedule list                                        # état ON/OFF + coût estimé/mois
cetic schedule create weekend --vm web-01 --off "fri:20-mon:08"   # OFF vendredi 20h → lundi 08h
cetic schedule create nuits --vm-scale-set fleet \
    --off "mon:22-tue:07" --off "tue:22-wed:07" --timezone Europe/Paris
cetic schedule create pool-nuit --ccks-node-pool prod gpu --off "mon:20-tue:08"
cetic schedule disable weekend                             # suspendre temporairement
cetic schedule delete weekend                              # supprime + rallume la cible
